...
def bright() :
    ...
    for i in range(args.amount) :
        for j in range(args.num) :
            image = Image.open('template2\\tmp2_res\\rotate\\pic' + str(i) + '.jpg')
            # 亮度增强
            enh_bri = ImageEnhance.Brightness(image)
            cc1 = random.uniform(1.0 - args.bright_limit, 1.0 + args.bright_limit)  # 亮暗系数
            brightness = cc1
            image_brightened = enh_bri.enhance(brightness)
            image_brightened.save('template2\\tmp2_res\\bright\\pic' + str(i) + '_' + str(j) + '_bri.jpg')
...
def color() :
    ...
    for i in range(args.amount) :
        for j in range(args.num) :
            ...
            image_colored.save('template2\\tmp2_res\\color\\pic' + str(i) + '_' + str(j) + '_col.jpg')
...
def contrast() :
    ...
    for i in range(args.amount) :
        for j in range(args.num) :
            ...
            image_contrasted.save('template2\\tmp2_res\\contrast\\pic' + str(i) + '_' + str(j) + '_con.jpg')
...
def sharp() :
    ...
    for i in range(args.amount) :
        for j in range(args.num) :
            ...
            image_sharped.save('template2\\tmp2_res\\sharp\\pic' + str(i) + '_' + str(j) + '_sha.jpg')
                ...
                for y in range(x1, x2) :
                    for z in range(c) :
                        if (img[x, y, z] + dark) > 255 :
                            ...
                            img[x, y, z] += dark
            cv2.imwrite('template2\\tmp2_res\\part_bright\\pic' + str(i) + '_' + str(j) + '_part.jpg', img)
